...
export const fetchContentList = async (search: string) => {
  try {
    const response = await axios.post(
      `https://api-sabi.langitdigital78.com/api/v1/get/data`,
      {
        query: search,
...
export const fetchContentKKList = async (search: string) => {
  try {
    const response = await axios.get(
      `https://api-secret-kk.langitdigital78.com/kartu-keluarga?search=${search}`
    );
